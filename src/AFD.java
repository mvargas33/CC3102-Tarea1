...
public class AFD {
    ...
    /*
    public void AFNDtoAFD(ArrayList<State> k_nd){
        int n = k_nd.size();                                            // Se calcula el número de estados en la lista
        if (n == 0){                                                    // Caso base: Si no hay estados se retorna
            return;
        }
        for (int x = 0; x < n; x++){                                    // Para cada estado en la lista de estados
            State state = k_nd.get(x);                                  // Se rescata el estado
            QState qState = new QState(this.QStates.size(), state);     // Se crea un estado cúantico con el estado como base
            qState.getStates().addAll(scope(state));                    // Se añaden las transiciones epsilon al estado cuantico
            this.QStates.add(qState);                                   // Se añade el estado cuantico a AFD
            QStateRecursion(qState);                                    // Se generan los estados cuanticos creados a partir de qState
        }
...
/*
    public void AFNtoAFD(ArrayList<State> k_nd){
        int n = k_nd.size();                                            // Se calcula el número de estados en la lista
        if (n == 0){                                                    // Caso base: Si no hay estados se retorna
            return;
        }
        for (int x = 0; x < n; x++){                                    // Para cada estado en la lista de estados
            ...
            for(int  j= 0; j < qState.getStates().size(); j++){         // Para cada estado en la lista de estados del estado cuantico
                State firststate = qState.getStates().get(j);           // Se rescata el estado
